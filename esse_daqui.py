...
cred = credentials.Certificate(r'C:\Users\F04\Downloads\Elei-oFocus-main\arquivo-credenciais.json')
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://votacao-fb7ae-default-rtdb.firebaseio.com/'  # Substitua pelo URL do seu Realtime Database
})

# Função para puxar dados do Realtime Database
def baixar_dados_do_firebase():
    print("Puxando dados do Firebase...")
    
    # Acessando os dados
    ref = db.reference('municipios/2020/13501/acessosDisponiveis')  # Ajuste o caminho conforme necessário
    dados = ref.get()
    
    # Verifique se dados existem
    if dados is None:
        print("Nenhum dado encontrado.")
        return pd.DataFrame()  # Retorne um DataFrame vazio se não houver dados
    
    # Convertendo os dados em um DataFrame do pandas
    dados_lista = [{"acessosDisponiveis": dados}]
    
    dados_df = pd.DataFrame(dados_lista)
    print(f"Número de documentos baixados: {len(dados_df)} registros")  # Exibe a contagem
...
def main():
    ...
    dados = baixar_dados_do_firebase()
